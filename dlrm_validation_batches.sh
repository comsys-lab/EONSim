...
# dataset_list=("dlrm/reuse_high_table_500K.txt")
...
BS_INCREMENT=${2:-8}     # N: batch size increment (default 8)
MAX_BS=${3:-32}          # M: maximum batch size (default 32)

# Set PROF_MULTIPLIER to $4 if provided, otherwise default to 1
PROF_MULTIPLIER=${4:-1}
##############################

### mNPUsim-related parameters ###
OFFMEM_CFG="dram_config/total_dram_config/single_hbm3_819gbs.cfg"
# OFFMEM_CFG="dram_config/total_dram_config/single_hbm2_512gbs.cfg"
...
for dataset in "${dataset_list[@]}"; do
    ...
    # Iterate through batch sizes from BS_INCREMENT to MAX_BS
    for (( BS=32; BS<=MAX_BS; BS+=BS_INCREMENT )); do
        ...
        for e in $EMBS; do
            ...
            echo "Running simulation with BATCH_SIZE=$BS"
