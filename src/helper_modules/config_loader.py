...
class ConfigLoader:
    ...
    def get_embedding_config(self):
        emb_config = self.config.get('embedding_vector_operation', {})
    ...
    def get_matrix_ops_config(self):
        mo = self.config.get('matrix_operation')
        if not mo:
            return None
        return {
            'format': mo.get('format', 'gemm'),
            'layers_text': mo.get('layers', ''),
        }
