...
string custom_trace_path = "";
bool use_custom_trace = false;
bool custom_trace_copied = false;
...
void read_network_config(npu_accelerator *ng, string file_name)
{
	...
	while(!file.eof())
	{
		...
		getline(file, str_buf, ',');
		if (str_buf.find_first_not_of("0123456789") != string::npos) {
			cout << "Skipping invalid line with non-numeric data: " << str_buf << endl;
			getline(file, str_buf); // skip rest of this line
			continue;
		}
