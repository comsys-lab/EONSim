...
{
	// Check if using custom trace
	if(use_custom_trace && !custom_trace_path.empty()) {
		// Only copy custom trace on first call
		if (!custom_trace_copied) {
			ifstream custom_file(custom_trace_path);
			if (custom_file.is_open()) {
				string trace_content((istreambuf_iterator<char>(custom_file)),
				                     istreambuf_iterator<char>());
				custom_file.close();
				
				// Write custom trace as dram_read intermediate file
				write_output(result_path, dram_read_intermediate, trace_content);
				cout << "Using custom trace for intermediate file: " << dram_read_intermediate << endl;
				
				// Create minimal dram_write file with dummy data
				write_output(result_path, dram_write_intermediate, "0,");
				
				// Create minimal SRAM files with dummy data
				write_output(result_path, sram_read_filter_intermediate, "0,");
				write_output(result_path, sram_read_ifmap_intermediate, "0,");
				write_output(result_path, sram_write_ofmap_intermediate, "0,");
				
				custom_trace_copied = true;
				return;
			} else {
				cout << "Warning: Could not open custom trace file: " << custom_trace_path << endl;
				cout << "Falling back to normal trace generation..." << endl;
			}
		} else {
			// Skip subsequent calls when using custom trace
			return;
		}
	}

	// Original trace generation code
	string dram_string = "";
